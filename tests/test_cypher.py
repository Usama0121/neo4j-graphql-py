...
class TestSchema(unittest.TestCase):
    ...
    def augmented_schema_test(self, graphql_query, expected_cypher_query, params=None):
    ...
    def test_add_relation_mutation(self):
        ...
        '''
        expected_cypher_query = ('MATCH (movie:Movie {movieId: $moviemovieId}) '
    ...
    def test_add_relation_mutation_with_graphql_variables(self):
        ...
        expected_cypher_query = ('MATCH (movie:Movie {movieId: $moviemovieId}) '
                                 ...
                                 'RETURN movie {_id: ID(movie), .title ,genres: '
                                 '[(movie)-[:IN_GENRE]->(movie_genres:Genre {}) | movie_genres { .name }] } AS movie')
        self.cypher_test(graphql_query, expected_cypher_query, params={'movieParam': '123'})
    ...
    def test_handle_graphql_variables_in_nested_selection_first(self):
        ...
        expected_cypher_query = ('MATCH (movie:Movie {year: 2016}) RETURN movie { .title , .year ,'
                                 ...
                                 'MATCH (this)--(:Genre)--(o:Movie) RETURN o", {this: movie, first: 3, offset: 0}, '
                                 'true) | movie_similar { .title }][..3] } AS movie SKIP 0')
        self.cypher_test(graphql_query, expected_cypher_query, params={'year': 2016, 'first': 3})
        self.augmented_schema_test(graphql_query, expected_cypher_query, params={'year': 2016, 'first': 3})
    ...
    def test_handle_graphql_variables_in_nested_selection_cypher(self):
        ...
        self.cypher_test(graphql_query, expected_cypher_query, params={'year': 2016, 'first': 3, 'scale': 5})
        self.augmented_schema_test(graphql_query, expected_cypher_query, params={'year': 2016, 'first': 3, 'scale': 5})
