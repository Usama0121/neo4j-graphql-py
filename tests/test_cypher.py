...
from neo4j_graphql_py import cypher_query
...
class Test(unittest.TestCase):
    ...
    def _test_runner(self, graphql_query, expected_cypher_query):
        ...
        def resolve_any(obj, info, **kwargs):
    ...
    def test_simple_cypher_query(self):
        ...
        expected_cypher_query = 'MATCH (movie:Movie {title: "River Runs Through It, A"}) RETURN movie { .title } AS movie SKIP 0'
    ...
    def test_simple_skip_limit(self):
        graphql_query = '''{
                ...
                '''
        expected_cypher_query = 'MATCH (movie:Movie {title: "River Runs Through It, A"}) RETURN movie { .title , .year } AS movie SKIP 0 LIMIT 1'
        self.base_test(graphql_query, expected_cypher_query)
    ...
    def test_cypher_projection_skip_limit(self):
        ...
        expected_cypher_query = 'MATCH (movie:Movie {title: "River Runs Through It, A"}) RETURN movie { .title ,actors: [(movie)<-[:ACTED_IN]-(movie_actors:Actor) | movie_actors { .name }] ,similar: [ x IN apoc.cypher.runFirstColumn("WITH {this} AS this MATCH (this)--(:Genre)--(o:Movie) RETURN o", {this: movie}, true) | x { .title }][..3] } AS movie SKIP 0'
        self.base_test(graphql_query, expected_cypher_query)
    ...
    def test_query_single_object_relation(self):
        graphql_query = '''
              ...
              '''
        expected_cypher_query = 'MATCH (movie:Movie {movieId: "3100"}) RETURN movie { .title ,filmedIn: head([(movie)-[:FILMED_IN]->(movie_filmedIn:State) | movie_filmedIn { .name }]) } AS movie SKIP 0'
        self.base_test(graphql_query, expected_cypher_query)
    ...
    def test_query_single_object_array_of_objects_relations(self):
        ...
        expected_cypher_query = 'MATCH (movie:Movie {movieId: "3100"}) RETURN movie { .title ,actors: [(movie)<-[:ACTED_IN]-(movie_actors:Actor) | movie_actors { .name }] ,filmedIn: head([(movie)-[:FILMED_IN]->(movie_filmedIn:State) | movie_filmedIn { .name }]) } AS movie SKIP 0'
        self.base_test(graphql_query, expected_cypher_query)
